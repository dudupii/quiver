...
- **pi**: the core spawns no agents of its own — the platform's documented path is a second pi instance via tmux. If a background-agent extension package is installed, defer to it per its own docs. Otherwise, when tmux is available run `tmux new-session -d -s "<slug>" 'pi -p --name "<name>" "<seed>"'` — the tmux session takes a slug of the descriptive name (tmux rejects `.` and `:` in session names, and a `#` unquoted would comment the command away: spaces become dashes, everything else non-alphanumeric drops, e.g. `quiver #12 relay arrow` → `quiver-12-relay-arrow`), while `pi --name` carries the full descriptive name; `pi -p` is the headless one-shot run, the user watches progress with `tmux attach -t <slug>` and revisits the saved session via `pi --resume`. Without tmux, report unsupported and hand over the seed for a manual launch.
